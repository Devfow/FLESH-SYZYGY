...
function draw() {
    ...
    const applyJitter = (x, y) => {
        ...
        return { x: x + ox + shakeOffsetX, y: y + oy + shakeOffsetY };
    };

    // Draw map
    for (let x = 0; x < mapWidth; x++) {
        for (let y = 0; y < mapHeight; y++) {
            const { x: jx, y: jy } = applyJitter(x * tileSize, y * tileSize);
            ctx.fillStyle = map[x][y].color;
            ctx.fillText(map[x][y].char, jx, jy);
        }
    }

    // Draw enemies
    for (const enemy of enemies) {
        const { x: jx, y: jy } = applyJitter(enemy.x * tileSize, enemy.y * tileSize);
        ctx.fillStyle = enemy.color;
        ctx.fillText(enemy.char, jx, jy);
    }

    // Draw items
    for (const item of items) {
        const { x: jx, y: jy } = applyJitter(item.x * tileSize, item.y * tileSize);
    ...
    for (const corpse of corpses) {
        const { x: jx, y: jy } = applyJitter(corpse.x * tileSize, corpse.y * tileSize);
        ctx.fillStyle = corpse.color;
        ctx.fillText(corpse.char, jx, jy);
    }

    // Draw player
    const { x: jpx, y: jpy } = applyJitter(player.x * tileSize, player.y * tileSize);
    ctx.fillStyle = player.color;
    ctx.fillText(player.char, jpx, jpy);

    // Draw health and attack
    ctx.fillStyle = '#fff';
    ctx.fillText(`Health: ${player.health}/${player.maxHealth}`, 10, 20);
    ctx.fillText(`Attack: ${player.attack}`, 10, 40);
    ctx.fillText(`Biocurrency: ${player.biocurrency}`, 10, 60);
    ctx.fillText(`Level: ${player.level}`, 10, 80);
    if (player.weapon) {
        ctx.fillText(`Weapon: ${player.weapon.name} (${player.weapon.durability})`, 10, 100);
    }

    // Draw message log
    for (let i = 0; i < messageLog.length; i++) {
        ctx.fillText(messageLog[i], 10, canvas.height - (messageLog.length - i) * 20 - 10);
    }
}
